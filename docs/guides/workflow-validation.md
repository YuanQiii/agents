...
| workflow                                          | 触发                                                                   | 内容                                                                                                                                                                                                                                              |
| ------------------------------------------------- | ---------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `.github/workflows/linter.yml`                    | PR → `main`；另加 `schedule`（每周一 09:00 UTC）与 `workflow_dispatch` | `github/super-linter`，`VALIDATE_ALL_CODEBASE: false`（只查改动），排除 `docs` 与 `.github`，且 `VALIDATE_MARKDOWN: false`。**`schedule` 触发时这个 job 会被跳过**（`if: github.event_name != 'schedule'`），每周那次只跑零依赖的 `agents-md` job |
